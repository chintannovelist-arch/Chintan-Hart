...
const TropeMatchmaker: React.FC = () => {
    ...
    const handleMatch = async () => {
        ...
        setIsLoading(true);
        setResult(""); 
        const response = await callGeminiTropeMatch(input);
        setResult(response);
        setIsLoading(false);
    };
                ...
                <div className="grid md:grid-cols-2 gap-20 items-center">
                    <div className="space-y-8">
                         <div className="inline-flex items-center gap-3 px-4 py-1 bg-white/5 border border-white/10 rounded-sm text-primary text-[10px] tracking-[0.3em] uppercase">
                            <Search size={12} /> <span>Trope Matchmaker</span>
                        </div>
                        ...
                        <p className="text-slate-400 text-lg leading-loose font-body font-light max-w-md">
                            Tell us your favorite romance trope—be it <em>"One Bed"</em>, <em>"Forced Proximity"</em>, or <em>"Acts of Service."</em> Our AI will scan the novel to find the exact chapter that satisfies your craving.
                        </p>
                        ...
                        <div className="relative bg-black/60 backdrop-blur-2xl border border-white/10 rounded-sm p-10 md:p-12 h-full flex flex-col justify-center shadow-glass transition-transform duration-700 border-t-white/20">
                            {isLoading ? (
                                <div className="animate-pulse space-y-6">
                                    ...
                                    <p className="text-center text-xs text-slate-500 tracking-widest uppercase animate-pulse">Scanning Manuscript...</p>
                                </div>
                            ) : result ? (
                                ...
                                <div className="text-center text-slate-600 flex flex-col items-center gap-6 opacity-60">
                                    ...
                                    <p className="font-display text-2xl tracking-wide">Search the archives...</p>
                                </div>
                            )}
                        </div>
                    </div>
...
export default TropeMatchmaker;
